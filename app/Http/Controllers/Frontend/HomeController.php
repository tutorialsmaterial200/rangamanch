...
class HomeController extends Controller
{
    public function index()
    {
        $breakingNews = News::where(['is_breaking_news' => 1,])
            ->activeEntries()->withLocalize()->orderBy('id', 'DESC')->take(5)->get();
        ...
        // Get single featured post (latest post with highest views)
        $singlePost = News::where(['is_breaking_news' => 1,])
            ->activeEntries()
            ->withLocalize()
        ...
        // Get Samacara category news
        $samacaraCategory = Category::where('name', 'समाचार')
            ->orWhere('name', 'Samacara')
            ->orWhere('slug', 'samacara')
            ->first();

        if ($samacaraCategory) {
            $categorySectionOne = News::with(['category', 'auther'])
                ->where('category_id', $samacaraCategory->id)
                ->activeEntries()
                ->withLocalize()
                ->orderBy('created_at', 'DESC')
                ->take(8)
        ...
        $bhadayaCategory = Category::where('name', 'भिडियो')
            ->orWhere('name', 'Bhadaya')
            ->orWhere('slug', 'bhadaya')
            ->first();

        if ($bhadayaCategory) {
            $categorybhadaya = News::with(['category', 'auther'])
                ->where('category_id', $bhadayaCategory->id)
                ->activeEntries()
                ->withLocalize()
                ->orderBy('created_at', 'DESC')
                ->take(8)
            ...
            $categorybhadaya = collect();
        }
  $manarajanaCategory = Category::where('name', 'मनोरंजन')
            ->orWhere('name', 'Manarajana')
            ->orWhere('slug', 'manarajana')
            ->first();
       if ($samacaraCategory) {
            $categorySectionTwo = News::with(['category', 'auther'])
                ->where('category_id', $manarajanaCategory->id)
                ->activeEntries()
                ->withLocalize()
                ->orderBy('created_at', 'DESC')
                ->take(8)
        ...
        // Get latest videos
        $latestVideos = Video::where('status', 1)
            ->orderBy('created_at', 'DESC')
            ->take(8)
            ->get();
