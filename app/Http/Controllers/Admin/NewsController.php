...
class NewsController extends Controller
{
    ...
    {
        try {
            /** Handle image - check if gallery image was selected */
            $imagePath = null;
            ...
            Log::error('News creation error: ' . $th->getMessage(), ['exception' => $th]);
            toast(__('admin.Error: ') . $th->getMessage(), 'error')->width('330');
    ...
    /**
     * Fetch gallery images for image selection modal
     */
    public function galleryImages(Request $request)
    {
        ...
        if (is_dir($uploadsPath)) {
            ...
            foreach ($files as $file) {
                ...
                // Check if it's a file and is an image
                if (is_file($filePath)) {
                    ...
                    if (strpos($mimeType, 'image') === 0) {
                        $images[] = [
                            ...
                            'uploaded_at' => date('Y-m-d H:i', filemtime($filePath))
                        ];
                    }
                }
            }
